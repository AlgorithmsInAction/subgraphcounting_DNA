...
usage() {
  ...
  echo "Writes one summary row with only the total time for triangle updates." >&2
}
...
output_path="${2:-$(default_output_path "$input_path" ".triangle_total_time_stats.csv")}"
ensure_parent_dir "$output_path"

ensure_built
run_signed_counter "$input_path" \
  --timing-mode aggregate-only \
  --metric-set triangles \
